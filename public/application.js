...
var config = {
  apiOrigins: {
    ...
    // original  production: 'https://ga-wdi-boston.herokuapp.com'
...
var displayGameTurn = function displayGameTurn(turn) {
  $('#message3a').html(turn);
};

var sectorIsOccupied = function sectorIsOccupied() {
...
var showGameSuccess = function showGameSuccess() {
...
var showGameStatsSuccess = function showGameStatsSuccess(object) {
...
var displayWinnner = function displayWinnner(playerValue) {
...
var showScore = function showScore(xScore, oScore) {
...
module.exports = {
  ...
  showGameStatsSuccess: showGameStatsSuccess,
  showGameStatsFailure: showGameStatsFailure,
  displayGameTurn: displayGameTurn,
  sectorIsOccupied: sectorIsOccupied,
  boardFull: boardFull,
  displayToken: displayToken,
  displayWinnner: displayWinnner,
...
var saveGameState = function saveGameState(event) {
  event.preventDefault();
  //  console.log('saveGameState ran!')

  var data = getFormFields(this);
  api.changePassWord(data).then(ui.saveGameSuccess).catch(ui.saveGamFailure);
};

// code for Game UI handlers
// let's just get the grid to update
var clickSector = function clickSector(event) {
  //  console.log('clickSector called')
  var boardCell = $(event.target).attr('id');
  event.preventDefault();
  // get the current turn
  var num = window.gameTurn;
  ...
  if (gameLogic.currentTurn(num) === true) {
    ...
    var sectorCheck = gameLogic.updateBoard(boardCell, 'o');
    if (sectorCheck === true) {
    ...
    var _sectorCheck = gameLogic.updateBoard(boardCell, 'x');
    if (_sectorCheck === true) {
      // console.log('events.js', sectorCheck)
...
var displayGameStats = function displayGameStats() {
  event.preventDefault();
  api.getGameStats().then(ui.showGameStatsSuccess).catch(ui.showGameStatsFailure);
};
...
// get game info
var getGameStats = function getGameStats(data) {
...
var currentGameTurn = function currentGameTurn() {
  gameTurn++;
  return gameTurn;
};
...
var playerOPoint = 0;
// score counter
var currentScore = function currentScore(player) {
  if (player === 'x') {
...
var occupiedSector = function occupiedSector(index) {
  var array = gameBoard;
...
var updateBoard = function updateBoard(index, value) {
  ...
  var checkSector = occupiedSector(index);
...
// Turn checker
var currentTurn = function currentTurn(turn) {
